...
<x-guest-layout>
    <div class="w-full max-w-xs sm:max-w-sm md:max-w-lg ">
        ...
        <div class="card-glow shadow-2xl hover:shadow-3xl dark:shadow-gray-900/50 bg-stone-200/90 dark:bg-custom-gray rounded-2xl transition-all duration-300">
            ...
            <div class="px-3 sm:px-4 md:px-5 py-1 sm:py-2 pb-3 sm:pb-4">
                <h2 class=" text-lg sm:text-2xl md:text-3xl lg:text-3xl text-center text-custom-dark dark:text-amber-100 font-black transition-colors">Recuperación de Contraseña</h2>
            </div>
            ...
            <div class="px-3 sm:px-4 md:px-5 sm:pb-4">
                ...
                <form method="POST" class="space-y-2.5 sm:space-y-3 md:space-y-5" action="{{ route('password.email') }}">
                    @csrf
                    ...
                    <div class="flex items-center justify-end mt-4 space-x-6">
                        <a href="{{ route('login') }}" class="relative inline-flex items-center px-4 py-1 bg-red-600 dark:bg-red-700 text-white font-bold rounded-md hover:opacity-95 dark:hover:opacity-90 transition-all duration-200 shadow-sm hover:shadow-md focus:scale-[1.03] hover:scale-[1.03] active:scale-95 before:content-[''] before:absolute before:inset-0 before:rounded-md before:ring-[3px] before:ring-offset-2 before:ring-offset-grey-400 before:ring-red-600 dark:before:ring-offset-gray-950 dark:before:ring-red-700">
                            Cancelar
                        </a>
                        <button type="submit" class="relative [background:linear-gradient(144deg,#3E2723,#4E342E_50%,#6D4C41)] dark:[background:linear-gradient(144deg,#3b220f,#61361d_50%,#794606)] text-white px-4 py-1 font-bold rounded-md hover:opacity-95 dark:hover:opacity-90 transition-all duration-200 shadow-sm hover:shadow-md focus:scale-[1.03] hover:scale-[1.03] active:scale-95 before:content-[''] before:absolute before:inset-0 before:rounded-md before:ring-[3px] before:ring-offset-2 before:ring-offset-[#f1ae6dd1] before:ring-[#3E2723] dark:before:ring-offset-gray-950 dark:before:ring-amber-600/70">
                            {{ __('Enviar enlace') }}
                        </button>
                    </div>
                </form>
